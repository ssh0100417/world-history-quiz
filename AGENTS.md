...
- 최종 출제 버전 선택 UI에는 `V0`, `V3`, `V4`가 있어야 한다.
...
- V4를 추가할 때는 `QUESTION_BANK`, `VERSION_OPTIONS`, `selectedVersions` 기본값에 V4가 반영되어야 한다.
...
- 중복 ID가 없는지 확인
- `VERSION_OPTIONS`에 `V4`가 포함되는지 확인
- `selectedVersions` 기본값에 `V4`가 포함되는지 확인
- `QUESTION_BANK`가 `V0 + V3 + V4`를 포함하는지 확인
