...
- V0는 유지한다.
- V3는 V3 기존 문제와 V4의 중복되지 않은 문제를 병합한 버전이다.
- V4는 별도 출제 버전으로 남기지 않는다.
- 최종 출제 버전 선택 UI에는 `V0`, `V3`만 남아야 한다.
- `V3`를 선택하면 병합된 V3 전체 문항이 출제 후보에 포함되어야 한다.
- `V4_QUESTION_BANK`가 출제 후보에 포함되면 안 된다.
...
- 기존 V3 문제는 유지한다.
- 기존 V4 문제는 주차별로 기존 V3 문제와 비교한다.
- 완전히 중복되는 문제만 제외한다.
- 비슷한 주제이거나 같은 사건을 다룬다는 이유만으로 중복 처리하지 않는다.
- 중복되지 않는 V4 문제는 `version`을 `V3`로 바꿔 V3 문제은행에 병합한다.
- 병합 후 V4는 별도 출제 버전으로 남기지 않는다.
...
- 모든 병합 문제의 `version`은 반드시 `V3`이어야 한다.
...
- V3/V4 병합만 수정한 경우 기본 커밋 후보는 다음이다.
...
- V4 객체가 남아 있지 않은지 확인
- `V4_QUESTION_BANK`가 남아 있지 않은지 확인
- V3 ID가 주차별로 연속되는지 확인
- 중복 ID가 없는지 확인
- `VERSION_OPTIONS`에 `V4`가 없는지 확인
- `selectedVersions` 기본값에 `V4`가 없는지 확인
- `QUESTION_BANK`가 `V0 + V3`만 포함하는지 확인
